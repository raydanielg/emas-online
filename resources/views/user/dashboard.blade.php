...
@section('content')
    ...
    @elseif($school && $school->status === 'Approved')
        <div class="card mb-3 emas-welcome-card">
            <div class="card-body emas-welcome-body">
                ...
                <div class="emas-welcome-title">{{ $school->name }}, welcome back</div>
    ...
    <div class="row">
        <div class="col-lg-3 col-6">
            <div class="small-box bg-info">
                ...
                <a href="{{ route('user.students.index') }}" class="small-box-footer">More info <i class="fas fa-arrow-circle-right"></i></a>
            </div>
        </div>
        <div class="col-lg-3 col-6">
            <div class="small-box bg-success">
                ...
                <a href="{{ route('user.classes.index') }}" class="small-box-footer">More info <i class="fas fa-arrow-circle-right"></i></a>
            </div>
        </div>
        <div class="col-lg-3 col-6">
            <div class="small-box bg-warning">
                ...
                <a href="{{ route('user.subjects.index') }}" class="small-box-footer">More info <i class="fas fa-arrow-circle-right"></i></a>
            </div>
        </div>
        <div class="col-lg-3 col-6">
            <div class="small-box bg-danger">
                ...
                <a href="{{ route('user.exams.index') }}" class="small-box-footer">More info <i class="fas fa-arrow-circle-right"></i></a>
            </div>
        </div>
    </div>
                        ...
                        <table class="table table-striped table-hover mb-0">
                            ...
                            <tbody>
                                @forelse(($classStats ?? []) as $row)
                                    <tr>
                                        <td class="font-weight-bold">{{ $row['class_level'] }}</td>
                                        <td class="text-center">{{ $row['total'] }}</td>
                                        <td class="text-center">{{ $row['male'] }}</td>
                                        <td class="text-center">{{ $row['female'] }}</td>
                                        <td class="text-center">
                                            @if($row['without_dob'] > 0)
                                                <span class="badge badge-warning text-dark">{{ $row['without_dob'] }}</span>
                                            @else
                                        ...
                                        <td>
                                            <div class="d-flex align-items-center">
                                                ...
                                                <span class="badge badge-success ml-2">{{ $row['progress'] }}%</span>
                                            </div>
                                        </td>
                                    </tr>
                                @empty
                    ...
                    <div class="d-flex justify-content-between align-items-start">
                        <div>
                            ...
                            <div class="h4 font-weight-bold mb-0">
                                {{ ($overview['total_students'] ?? 0) - ($overview['without_dob'] ?? 0) }} / {{ $overview['total_students'] ?? 0 }}
                            </div>
                            <div class="text-white-50">Students with DOB filled</div>
                        </div>
                        <div class="text-right">
                            <div class="h2 font-weight-bold mb-0">{{ $overview['completion_percent'] ?? 0 }}%</div>
                            <div class="text-white-50">Completion</div>
                        </div>
                    </div>
                    <div class="progress mt-3" style="height: 10px; background: rgba(255,255,255,.2);">
                        <div class="progress-bar" role="progressbar" style="width: {{ $overview['completion_percent'] ?? 0 }}%; background: rgba(255,255,255,.9);"></div>
                    </div>
                </div>
            </div>
            ...
            <div class="card card-outline card-success shadow-sm">
                ...
                <div class="card-body">
                    <div class="d-flex justify-content-between">
                        <div class="text-muted">{{ $school?->name ?? 'School' }}</div>
                        <div class="font-weight-bold">
                            {{ ($overview['total_students'] ?? 0) - ($overview['without_dob'] ?? 0) }}/{{ $overview['total_students'] ?? 0 }}
                            <span class="text-muted">({{ $overview['completion_percent'] ?? 0 }}%)</span>
                        </div>
                    </div>
                    <div class="progress mt-2" style="height: 12px;">
                        <div class="progress-bar bg-info" role="progressbar" style="width: {{ $overview['completion_percent'] ?? 0 }}%"></div>
                    </div>

                    <div class="mt-3 small text-muted">
                        Missing DOB: <span class="font-weight-bold">{{ $overview['without_dob'] ?? 0 }}</span>
                    </div>
                    <div class="small text-muted">
                        Missing Middle Name: <span class="font-weight-bold">{{ $overview['without_middle_name'] ?? 0 }}</span>
                    </div>
                </div>
            </div>
        </div>
...
@section('js')
    ...
    <script>
        (function () {
            const chartData = @json($genderChart ?? ['labels' => [], 'male' => [], 'female' => []]);
