...
<body class="bg-gray-50 text-gray-900">
        ...
        <div class="container mx-auto px-6 relative z-10">
            <nav class="flex justify-between items-center mb-16">
                ...
                <div class="flex items-center space-x-4">
                    <a href="{{ route('public.results.index') }}" class="hidden lg:block px-4 py-2 text-sm btn-outline rounded-lg">Results</a>
                    <a href="{{ route('public.resources.index') }}" class="hidden lg:block px-4 py-2 text-sm btn-outline rounded-lg">Resources</a>
                    @if (Route::has('login'))
                        @auth
                            <a href="{{ url('/dashboard') }}" class="px-6 py-2 bg-white text-teal-900 font-bold rounded-lg hover:bg-gray-100 transition shadow-lg">Dashboard</a>
                        @else
                            <a href="{{ route('login') }}" class="px-6 py-2 btn-outline text-white font-bold rounded-lg transition border-white">Sign In</a>
                        @endauth
                    @endif
                </div>
            </nav>
            ...
            <div class="flex flex-col lg:flex-row items-center justify-between text-left">
                <div class="lg:w-2/3 mb-12 lg:mb-0">
                    ...
                    <div class="flex items-center space-x-3 md:hidden mb-8">
                        <a href="{{ route('public.results.index') }}" class="flex-1 text-center px-4 py-2 text-sm btn-outline rounded-lg">Results</a>
                        <a href="{{ route('public.resources.index') }}" class="flex-1 text-center px-4 py-2 text-sm btn-outline rounded-lg">Resources</a>
                    </div>
                </div>
    ...
    <footer class="bg-teal-950 text-white py-12 text-center">
        <div class="container mx-auto px-6">
            ...
            <p class="text-teal-400 text-sm opacity-80">&copy; {{ date('Y') }} <span class="text-yellow-600">e</span>MaS. All rights reserved.</p>
        </div>
    </footer>
</body>
</html>